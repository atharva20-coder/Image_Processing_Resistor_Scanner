...
def selectImage():
    ...
    if file_path:
        # Copy file to project root folder
        global file_name
        ...
        success_msg = customtkinter.CTkLabel(app, text=f"File '{file_name}' uploaded successfully!")
        success_msg.pack(padx=10, pady=10)

        # Display uploaded image and file name
        img_label = customtkinter.CTkLabel(app, text=f"Uploaded image: {file_name}")
        img_label.pack(padx=10, pady=10)
        img = ImageTk.PhotoImage(Image.open(dest_path))
        img_widget = customtkinter.CTkLabel(app, image=img)
        img_widget.pack(padx=10, pady=10)
        img_widget.image = img # This line is important to prevent garbage collection

        # Enable and show quitButton
        quitButton.pack()
...
app.title("Resistance Image Processing")

# Adding UI Elements
title = customtkinter.CTkLabel(app, text="Resistance Calculation using Image Processing")
title.pack(padx=10, pady=10)

# Image upload button
upload = customtkinter.CTkButton(app, text="Upload Resistor Image", command=selectImage)
upload.pack()

# Quit Window Button
quitButton = customtkinter.CTkButton(app, text="Click to view result", command=showCanvas)
...
if x_min >= 0: #if minx is greater than or equal to 0 (i.e. respective index of nums array saved above is higher than 0)
    ...
    if w > 10 and ratio < 0.7 and ratio > -0.7:
        ...
        tolerance_gold = "Gold: \xb15%"
        tolerance_silver = "Silver: \xb110%"
        tolerance_nocolor = "No Color: \xb120%"

        # Calculate text sizes
        result_text_size, _ = cv2.getTextSize(answer, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
        tolerance_gold_size, _ = cv2.getTextSize(tolerance_gold, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
        tolerance_silver_size, _ = cv2.getTextSize(tolerance_silver, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
        tolerance_nocolor_size, _ = cv2.getTextSize(tolerance_nocolor, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)

        # Calculate x-coordinates for all texts
        text_padding = 20
        result_text_x = 500
        tolerance_gold_x = 50 + result_text_size[0] + text_padding
        tolerance_silver_x = tolerance_gold_x + tolerance_gold_size[0] + text_padding
        tolerance_nocolor_x = tolerance_silver_x + tolerance_silver_size[0] + text_padding


        # Draw texts on image
        cv2.putText(image, answer, (result_text_x, image.shape[0] - 500), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
        cv2.putText(image, tolerance_gold, (tolerance_gold_x, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
        cv2.putText(image, tolerance_silver, (tolerance_silver_x, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
        cv2.putText(image, tolerance_nocolor, (tolerance_nocolor_x, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
