...
def selectImage():
    ...
    if file_path:
        ...
        img_label = customtkinter.CTkLabel(app, text=f"Your Resistor: {file_name}")
        ...
        # Enable and show calculateButton to process image
...
def processImage():
    ...
    if count > 0:  # if there are values greater than 0 in nums array
        ...
        for x in range(0, columns):
            if numbers_of_dark_pixels_in_each_column[
                x] > average_dark_pixels + 15:  # if nums(column number) is greater than ((average times the num has been greater than 0) + 15)
    ...
    if x_min >= 0:  # if minx is greater than or equal to 0 (i.e. respective index of nums array saved above is higher than 0)
        ...
        if w > 10 and ratio < 0.7 and ratio > -0.7:
            ...
            # Display uploaded image and file name
            tol_frame.pack()
            result_label = customtkinter.CTkLabel(app, text=f"Resistance is: {answer}")
...
# Quit Window Button
calculate_button = customtkinter.CTkButton(app, text="Click to view result", command=processImage)
calculate_button.pack_forget()

# Create a frame to hold the tolerance labels
tol_frame = customtkinter.CTkFrame(app)
tol_frame.pack(padx=10, pady=10)
tol_frame.pack_forget()

# Tolerance labels
tol = customtkinter.CTkLabel(tol_frame, text="Tolerences:")
tol.grid(row=0, column=0, padx=10, pady=10)

gold = customtkinter.CTkLabel(tol_frame, text="Gold: ±5%")
gold.grid(row=0, column=1, padx=10, pady=10)

silver = customtkinter.CTkLabel(tol_frame, text="Silver: ±10%")
silver.grid(row=0, column=2, padx=10, pady=10)

no_color = customtkinter.CTkLabel(tol_frame, text="No Color: ±20%")
no_color.grid(row=0, column=3, padx=10, pady=10)
